 ...
 */
 ...
 /* Copyright (C) 2026 AeroLink. Custom Source Engine Obfuscation. */
		...
		for _, elem := range elemsContainer.elems {
			if len(elem.packet) != MessageKeepaliveSize {
				...
				if padding := device.paddings.transport; padding > 0 {
					// Сдвигаем реальный пакет вправо, освобождая место под паддинг
					for i := len(elem.packet) - 1; i >= 0; i-- {
						elem.buffer[i+padding] = elem.buffer[i]
					}
					
					// --- НАЧАЛО: НАША ИГРОВАЯ МАСКИРОВКА AEROLINK ---
					if padding >= 6 {
						// 1. Ставим магические байты Source Engine (CS2/Rust) -> 0xFF 0xFF 0xFF 0xFF
						elem.buffer[0], elem.buffer[1], elem.buffer[2], elem.buffer[3] = 0xFF, 0xFF, 0xFF, 0xFF
						
						// 2. Ставим номер тика (sequence), чтобы DPI видел динамику
						seq := atomic.AddUint32(&globalGameSeq, 1)
						binary.BigEndian.PutUint16(elem.buffer[4:6], uint16(seq))
						
						// 3. Остаток добиваем мусором (имитация игровых данных)
						rand.Read(elem.buffer[6:padding])
					} else {
						// Фолбэк, если паддинг в конфиге почему-то задан меньше 6 байт
